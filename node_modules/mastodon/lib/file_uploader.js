...
/**
 * FileUploader class used to upload a file to mastodon via the /media/upload (chunked) API.
 * Usage:
 *   var fu = new FileUploader({ file_path: '/foo/bar/baz.mp4' }, masto);
 *   fu.upload(function (err, bodyObj, resp) {
 *     console.log(err, bodyObj);
 *   })
 *
 * @param  {Object}         params  Object of the form { file_path: String }.
 * @param  {Mastodon(object)}   masto    Mastodon instance.
 */
var FileUploader = function (params, masto) {
  assert(params)
...
/**
 * Upload a file to Mastodon via the /media/upload (chunked) API.
 *
...
FileUploader.prototype._finalizeMedia = function(media_id, cb) {
   ...
   *
   * @param  {String}   media_id        media_id_string received from Mastodon after sending INIT comand.
